    ...
    <template v-else-if="current">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div class="flex gap-2">
          ...
          <router-link
            v-if="
              ['AWAITING_PAYMENT_1', 'AWAITING_PAYMENT_2'].includes(current.status?.toUpperCase())
            "
          ...
          <BaseCard title="المستندات المرفقة" icon="folder">
            <div class="divide-y divide-outline-variant/20">
              ...
              >
                ...
                <button
                  type="button"
                  class="text-primary hover:bg-primary/10 p-2 rounded-full transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
                  title="تحميل"
                  :disabled="isDownloadingByDocumentId[doc.id] === true"
                  @click="downloadDocument(doc)"
                >
                  <AppIcon v-if="isDownloadingByDocumentId[doc.id] !== true" name="download" />
                  <AppIcon v-else name="progress_activity" class="animate-spin" />
                </button>
              </div>
            </div>
            <div
...
async function downloadDocument(doc) {
  ...
  try {
    const response = await researchService.getDocumentSignedUrl(current.value.id, doc.id);
